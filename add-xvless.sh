...
sed -i '$ ilocation /sal@u='"$user"'&p='"$uid"'&' /etc/nginx/conf.d/vps.conf
...
sed -i '$ i}' /etc/nginx/conf.d/vps.conf
vlesslink1="vless://${uuid}@${domain}:443/?tyepe=ws&encryption=none&host=bug.com&path=%2sal@u%3D${user}%26p%3D${uid}%26&security=tls&encryption=none&type=ws#${user}"
vlesslink2="vless://${uuid}@${domain}:80?path=%2Fsal@u%3D${user}%26p%3D${uid}%26&encryption=none&type=ws#${user}"
